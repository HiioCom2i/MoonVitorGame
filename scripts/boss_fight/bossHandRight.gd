...
func _ready() -> void:
	# Corrige a conexão para o sinal 'animation_finished'
...
func _on_hand_anim_animation_finished() -> void:
	...
	if hand_anim.animation == "death":
		emit_signal("attackStar_finished")  # Emite o sinal apenas para a animação "attack"
		print("Emitido sinal 'attackStar_finished' da mão direita.")
